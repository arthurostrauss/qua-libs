...
def construct_video_mode(x_element: str,
                         x_amplitude: float,
                         y_element: str,
                         y_amplitude: float,
                         resolution: int,
    ...
    with program() as video_mode:
            ...
            with for_(average, 0, average < number_of_averages, average + 1):
                ...
                with while_(completed_moves < resolution * (resolution - 1)):
                    ...
                    assign(movement_direction,
                           movement_direction * -1)  # *-1 as subsequent steps in the opposite direction
                    assign(moves_per_edge,
                           moves_per_edge + 1)  # moving one row/column out so need one more move_per_edge
        ...
        with stream_processing():
            for stream_name, stream in zip(['I', 'Q'], [I_stream, Q_stream]):
                # stream processing to average number_of_averages together repeatedly
                stream.buffer(resolution ** 2).buffer(number_of_averages).map(FUNCTIONS.average()).save(stream_name)
